<!-- gstack:coding-tasks -->
<!--
  Ready-to-paste OpenClaw AGENTS.md section, vendored verbatim from gstack's
  openclaw/agents-gstack-section.md (gstack v1.55.0.0 / docs/OPENCLAW.md).
  entrypoint.sh appends this (idempotently, between the markers) into the
  running agent's AGENTS.md so a dispatched "Run /review on <repo>" becomes a
  spawned Claude Code session over ACP — NOT a free-form gateway-agent answer.

  The ACP spawn itself (sessions_spawn runtime:"acp") is OpenClaw-native; this
  section is the dispatch contract that tells the orchestrator to use it.
-->
## Coding Tasks (gstack)
